...
public class CurrencyConversionController {
	...
	public CurrencyConversion convertCurrency(@PathVariable String from, @PathVariable String to, 
		...
		ResponseEntity<CurrencyConversion> restTempResponse = new RestTemplate()
				.getForEntity("http://localhost:8000/exchange-value/from/{from}/to/{to}", 
						CurrencyConversion.class, uriVariables);
		
		CurrencyConversion response = restTempResponse.getBody();
	...
	public CurrencyConversion convertCurrencyFeign(@PathVariable  String from, @PathVariable String to, 
			@PathVariable BigDecimal quantity) {
		CurrencyConversion response = this.proxy.retrieveExchangeValue(from, to);
		return new CurrencyConversion(from, to, response.getConversionMultiple(), quantity,
				quantity.multiply(response.getConversionMultiple()), response.getPort());
	}
}
